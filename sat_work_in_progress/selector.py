...
from hmiscsv30reader import HmisCsv30Reader
...
class Selector:#IGNORE:R0903
    ...
    def validate(self, instance_doc, shred=True): #IGNORE:R0201
        ...
        tests = [HUDHMIS28XMLTest(), HUDHMIS30XMLTest(), HmisCsv30Test(), JFCSXMLTest(), PARXMLTest()]
        ## readers = [VendorXMLReader(instance_doc), HUDHMIS28XMLReader(instance_doc), JFCSXMLInputReader(instance_doc)]  ## JOE
        readers = [HUDHMIS28XMLReader(instance_doc), HUDHMIS30XMLReader(instance_doc), HmisCsv30Reader(instance_doc), JFCSXMLInputReader(instance_doc), PARXMLInputReader(instance_doc)]
        results = []
        #for item in tests:
...
class HmisCsv30Test:
    '''There is no schema proper for CSV, just sanity check it manually.'''
    def __init__(self):
        global name
        name = 'HmisCsv30'
        print 'running the', name, 'test'
    ...
    def validate(self, instance_stream):
        ...
        try:
            ...
            print 'CSV File Error. There appears to be a malformed file.  ', error
            raise 
...
class HmisCsv30Reader(HMISXML28Reader):
    def __init__(self, instance_dirname):
        #self.reader = HmisCsv30Reader(instance_dirname)
        pass
