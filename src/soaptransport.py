...
<soapenv:Envelope xmlns:soapenv="http://www.w3.org/2003/05/soap-envelope"
    ...
    def send_soap_envelope(self, ccd_data, referredToProviderID, source_id):
        ...
        try:
            need_notes = ' '.join([c.text for c in ccd.iter('{https://raw.githubusercontent.com/211tbc/synthesis/relaxed-airs-code/src/xsd/TBC_Extend_HUD_HMIS.xsd}NeedNotes')])
        except:
            need_notes = ''

        # FBY New 2017-04-18 : AIRS Code text
        try:
            airs_code = ' '.join([c.text for c in ccd.iter('{https://raw.githubusercontent.com/211tbc/synthesis/relaxed-airs-code/src/xsd/TBC_Extend_HUD_HMIS.xsd}TempAirsCode')])
        except:
            airs_code = ''
        ...
        if need_notes != '':
            for tag in ccd.iter('{https://raw.githubusercontent.com/211tbc/synthesis/relaxed-airs-code/src/xsd/TBC_Extend_HUD_HMIS.xsd}NeedNotes'):
                tag.getparent().remove(tag)

        # FBY New 2017-04-18 : Strip AirsCode and ServiceEvent from XML
        if airs_code != '':
            for tag in ccd.iter('{https://raw.githubusercontent.com/211tbc/synthesis/relaxed-airs-code/src/xsd/TBC_Extend_HUD_HMIS.xsd}TempAirsCode'):
                tag.getparent().remove(tag)
            for tag in ccd.iter('{https://raw.githubusercontent.com/211tbc/synthesis/relaxed-airs-code/src/xsd/TBC_Extend_HUD_HMIS.xsd}TempServiceEvent'):
                tag.getparent().remove(tag)
