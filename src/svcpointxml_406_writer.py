...
class SVCPOINTXMLWriter(DBObjects.databaseObjects):
    ...
    def __init__(self, poutDirectory, processingOptions, debug=False, debugMessages=None):
    	...
    	# adding a debug switch that is managed in the conf/settings.py
        if settings.DEBUG:
            self.debug = settings.DEBUG
            #print "Debug switch is: %s" % self.debug
            print "logging to self.debugMessages"
    ...
    def processXML(self): # records represents whatever element you're tacking more onto, like entry_exits or clients
    	...
    	for self.person in Persons:
    	    ...
    	    if not self.person == None and self.outcomes == None and not self.person.person_legal_first_name_unhashed == None and not self.person.person_legal_last_name_unhashed == None :
	...
	# SBB20100511 removing this code it's from prior client i.e. not synthesis
...
#	But ... first we have to get a daily census dictionary populated and available to call
    ...
    def customizeEntryExit(self, entry_exit, EE):
    	...
    	#    exit_date.text = self.fixDate(EE.participation_dates_end_date)
    ...
    def customizeInfo_release(self, info_release):
    	# self.IR
    	'''
    	First 3 elements are required.  Next three are optional
    ...
    def customizeMember(self, member_element, member):
    	...
    	if member.person_id_unhashed != "":
    	    ...
    	    date_entered.text = self.fixDate(member.person_id_unhashed_date_collected)
    	else:
    	    client_id.text = member.person_id_hashed
    	
    	    date_entered = ET.SubElement(member_element, "date_entered")
    	    date_entered.text = self.fixDate(member.person_id_hashed_date_collected)
    ...
    def customizeMemberEE(self, EEMember_element, site_service_participation, member):
    	...
    	# SBB20100518 Removing this apparently Bowman doesn't like this (even though it validates)
    	# SBB20100520 Adding this back in.
    	entry_date = ET.SubElement(EEMember_element, "entry_date")
    	entry_date.text = self.fixDate(site_service_participation.participation_dates_start_date)
    	#
    	## FIXME, other elements are nullable.
    	#exit_date = ET.SubElement(EEMember_element, "exit_date")
    	#exit_date.text = self.fixDate(site_service_participation.participation_dates_end_date)
