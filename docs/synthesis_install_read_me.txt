...
1. Install python > = 2.6.  In linux, the best way is to do this through your system's package manager.   Most linux distros already have it though.

2.  Install a postgres database.  Get it from your package manager, then configure it.  Here are notes for Debian: http://codeghar.wordpress.com/2009/01/24/postgresql-83-on-debian-lenny/
...
3. Create a directory to share all buildout files and your Pylons project: 
~$ cd ~
~$ mkdir synthesis
~$ cd synthesis

4) Get the latest version of buildout's bootstrap script:
~/synthesis$ wget "http://svn.zope.org/*checkout*/zc.buildout/trunk/bootstrap/bootstrap.py"

5) Get the buildout script: 
~/synthesis$ wget "http://xsd.alexandriaconsulting.com/repos/trunk/synthesis/docs/buildout.cfg"

6) Now we have a clean buildout config. Let's bootstrap the buildout and run it:
