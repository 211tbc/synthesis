...
if thisVersion  == '2.5' or thisVersion  == '2.6':
	try:
...
class SVCPOINTXML20Writer():
	...
	def __init__(self, poutDirectory, debug=False, debugMessages=None):
		print "%s Class Initialized" % self.__class__.__name__
		if settings.DEBUG:
		...
		# SBB20090825 this initializes data elements of the XML Document - SvcPoint takes Client and EntryExit Information
		self.root_element = self.createDoc()
		self.clients = self.createClients(self.root_element)
		self.entry_exits = self.createEntryExits(self.root_element)
	...
	def write(self):
		# Dumps a file to file system?
		self.writeOutXML(root_element)		

	def processXML(self, records): # records represents whatever element you're tacking more onto, like entry_exits or clients
...
#		# Instead of generating a number (above), use the client number that is already provided in the legacy system
		# or
		#self.SystemID = self.xmlU.initializeSystemID(self.intakes['client number'])
		
		# start the clients

		if not self.intakes == None and self.outcomes == None:
			client = self.createClient(records)
			self.customizeClient(client)
			self.customizeClientPersonalIdentifiers(client,self.intakes)
...
#				# ECJ 20071119 First, make sure there is a client record in the <clients> section for this individual, 
#				#or else the entry/exit will not be associated with a person, just a system ID
#				client = self.createClient(records)
#				self.customizeClient(client)
#				self.customizeClientPersonalIdentifiers(client,self.outcom)
#				# ECJ 20071119 Then create an actual entry/exit record for this person in the <entry_exits> section
#				#entry_exits = root_element.entry_exits
...
#		removed since amount (for a need) is minoccurs=0 and it is not reflected in the csv		
...
#		reason_unmet = ET.SubElement(need, "reason_unmet")
#		reason_unmet.tail = "\n"
		
#		family_id = ET.SubElement(need, "family_id")
#		family_id.tail = "\n"
		
#		need_notes = ET.SubElement(need, "need_notes")
#		need_notes.text = ""
#		need_notes.tail = "\n"
...
#		This needs to be populated with  daily census.csv data using the date field + 1 day from that file 		
	...
	def customizeDynamiccontent(self, dynamiccontent):
		...
		# Is client homeless?
		if self.intakes['HomelessNow'] <> "":
			isclienthomeless = ET.SubElement(dynamiccontent, "isclienthomeless")
			isclienthomeless.attrib["date_added"] = datetime.now().isoformat()
			isclienthomeless.attrib["date_effective"] = self.fixDate(self.intakes['IntakeDate'])
			if self.intakes['HomelessNow'].lower() == "true":
				isclienthomeless.text = self.intakes['HomelessNow'].lower()
			if self.intakes['HomelessNow'].lower() == "false":
				isclienthomeless.text = self.intakes['HomelessNow'].lower()
			isclienthomeless.tail = "\n"
		
		if self.intakes['PhysDisb'] <> "":
			...
			hud_disablingcondition.attrib["date_effective"] = self.fixDate(self.intakes['IntakeDate'])
			hud_disablingcondition.text = self.pickList.getValue("ENHANCEDYESNOPickOption",str.strip(self.intakes['PhysDisb'].upper()))
			hud_disablingcondition.tail = "\n"
		...
		if self.intakes['Hours-Week'] <> "":
			...
			hud_hrsworkedlastweek.attrib["date_effective"] = self.fixDate(self.intakes['IntakeDate'])
			hud_hrsworkedlastweek.text = str.strip(self.intakes['Hours-Week'])
			hud_hrsworkedlastweek.tail = "\n"
		...
		if self.pickList.getValue("SexPick","male") <> "":
			...
			svpprofgender.attrib["date_effective"] = self.fixDate(self.intakes['IntakeDate'])
			svpprofgender.text = self.pickList.getValue("SexPick","male")
			svpprofgender.tail = "\n"

		# dob (Date of Birth)
		if self.convertIntegerToDateTime(self.intakes['DOB']) <> "":
			svpprofdob = ET.SubElement(dynamiccontent, "svpprofdob")
			svpprofdob.attrib["date_added"] = datetime.now().isoformat()
			svpprofdob.attrib["date_effective"] = self.fixDate(self.intakes['IntakeDate'])			
		...
		if self.intakes['Branch'] <> "" or self.convertIntegerToDate(self.intakes['EntranceDate']) <> None or self.convertIntegerToDate(self.intakes['DischargeDate']) <> None:
			...
			hud_militarybranchinfo.text = "\n"
			hud_militarybranchinfo.tail = "\n"
...
#		buildWorkhistoryAttributes(workhistorystart)
	...
	def writeOutXML(self, root):
		...
		tree.write(os.path.join(settings.OUTPUTFILES_PATH, "page.xml"))
...
if __name__ == "__main__":
	vld = SVCPOINTXML20Writer(os.path.join(os.getcwd(), settings.OUTPUTFILES_PATH))
	files = vld.processXML(vld.clients)
	files = vld.processXML(vld.entry_exits)
